---
title: 
  "**PSTAT100** Data Science Concepts and Analysis"
subtitle: Spring 2026
author: "John Robin Inston"
image: "assets/pstat100-thumbnail.svg"
date: "2026-03-16"
toc: true
title-block-banner: false
categories: [Current, UCSB, Data Science, Python]

about:
  template: trestles
  image-width: 16em
  image-shape: rounded
  id: hero-heading

---

::: {#hero-heading}
<div class="h1">Welcome to PSTAT100!</div>

My name is [John Inston](about.qmd) and I will be the instructor for this course, I am a 4th year Ph.D. candidate in the Department of Statistics and Applied Probability here at UC Santa Barbara.  Thank you all for taking this course. I hope that you find it both interesting and informative.

This course aims to provide an overview of key concepts in data science and the use of tools for data retrieval, analysis, visualization, and reproducible research in preparation for advanced data science courses.  Topics include an introduction to inference and prediction, principles of measurement, missing data, and notions of causality, statistical traps, and concepts in data ethics and privacy.

#### 🔍 Reference Material

The contents of this course was prepared using past teaching material provided by [Ethan P. Marzpan](https://epm027.github.io) as well as historical course material made available online by the UCSB Department of Statistics and Applied Probability.

:::

## 📚 Material

- **Lecture Notes:**
  - [Online lecture notes.](../../notes/pstat100-notes/00-introduction/index.qmd)
- **Course Readings:**
  - [Python Data Science Handbook](https://jakevdp.github.io/PythonDataScienceHandbook/)
  - [Learning Data Science](https://learningds.org/intro.html)
  - [Introduction to Statistical Learning](https://www.statlearning.com)
- **Helpful Resources:**
  - [Download VSCode.](https://code.visualstudio.com/)
  - [Download Python.](https://www.python.org/downloads/)
  - [Download Anaconda.](https://www.anaconda.com/download)
  - [Download Quarto.](https://quarto.org/docs/get-started/)
  - [Documentation: Quarto for PDF documents](https://quarto.org/docs/output-formats/pdf-basics.html)

## ✏️ Information

### Teaching Staff

| Name          | Role       | Email                 | Office Hours                    |
|---------------|------------|-----------------------|---------------------------------|
| John Inston   | Instructor | johninston@ucsb.edu   | SH 5431T R 1:00PM - 3:00PM      |
| Lauren Hughes | TA         | laurenhughes@ucsb.edu | SH 5421 T 3:30PM - 5:30PM       |
| Yuting Ma     | TA         | yutingma@ucsb.edu     | Zoom Meeting M 9:00AM - 11:00AM |
| Zhuojun Lyu   | TA         | zhuojun@ucsb.edu      | Zoom Meetng W 1:00PM - 3:00PM   |

### Instruction

Course instruction will comprise of **20 lectures** (held twice per week) and **10 programming labs** (held weekly).  

- **Lecture:** 
  - TR 11:00AM - 12:15PM ILP 1101
- **Labs:**
  - M 1:00PM - 1:50PM ILP 4107 - Zhoujun Lyu
  - M 2:00PM - 2:50PM ILP 3209 - Zhoujun Lyu
  - M 3:00PM - 3:50PM ILP 3209 - Yuting Ma
  - M 4:00PM - 4:50PM Girvetz Hall 2129 - Yuting Ma
  - M 5:00PM - 5:50PM ILP 3209 - Lauren Hughes
  - M 6:00PM - 6:50PM ILP 3205 - Lauren Hughes

![Interactive Learning Pavilion Location](../../assets/images/ilp-map.png)

### Assessments

You will be required to complete 10 lab worksheets which will be due for submission the following Friday.

You will be required to complete 3 assignments which will be due for submission approximately every 2 weeks. 

> ❗ The due dates for the assignments have changed and will be posted on Canvas.  Your first assignment will be released on Wednesday April 8th and due on Friday April 17th at the end of week 3.

You final assessment will be a group project which will comprise of cleaning and analyzing a data set of your choice.  You will be required to submit a project proposal in Week 5 specifying your data set as well as providing a rough project outline.

### Course Schedule

Please check this schedule regularly throughout the term as it is updated with the latest material and reading suggestions.

| Week | Date        | Topics                              | Materials |
|------|-------------|----------------------------------|-----------|
| 1    | Tue, Mar 31 | Course Information. <br> Introduction to Data Science. <br> Data Science Lifecycle |  [Lec 1 Slides](material/lec1/lec1.qmd) <br> [Lec 2 Slides](material/lec2/lec2.qmd) <br> Lab 1 |
| 2    | Tue, Apr 7  | Sampling and Bias. <br> Data Preparation.  | [Lec 3 Slides](material/lec3/lec3.qmd) <br> [Lec 4 Slides](material/lec4/lec4.qmd)|
| 3    | Tue, Apr 14 | Visualizations. <br> Exploratory Data Analysis. | [Lec 5 Slides](material/lec5/lec5.qmd) <br> [Lec 6 Slides](material/lec6/lec6.qmd)  |
| 4    | Tue, Apr 21 | Probability Theory. <br> Statistical Foundations. <br> Sampling Distributions. | [Lec 7 Slides](material/lec7/lec7.qmd) <br> [Lec 8 Slides](material/lec8/lec8.qmd) |         
| 5    | Tue, Apr 28 | Simple Linear Regression. <br> OLS Estimation. | [Lec 9 Slides](material/lec9/lec9.qmd) <br> [Lec 10 Slides](material/lec10/lec10.qmd)  |          
| 6    | Tue, May 5  | Multiple Linear Regression. <br> Interaction Terms.     | [Lec 11 Slides](material/lec11/lec11.qmd) <br> Lec 12 Slides |          
| 7    | Tue, May 12 | Model Selection. <br> Regularization.   | Lec 13 Slides <br> Lec 14 Slides          |
| 8    | Tue, May 19 | Classification. <br> Decision Trees. <br> Random Forests.         | Lec 15 Slides <br> Lec 16 Slides        |          
| 9    | Tue, May 26 | Clustering. <br> Time Series Analysis.         |              |
| 10   | Tue, Jun 2  | Neural Networks. <br> Deep Learning.         |      |








