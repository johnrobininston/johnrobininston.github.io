...
local function text_to_inlines(text)
  ...
  local i, n = 1, #text
  while i <= n do
    -- LOCAL PATCH: match ASCII whitespace only, not Lua's locale-dependent
    -- "%s". On some locales "%s" matches byte 0xA0 (the trailing byte of a
    -- UTF-8 non-breaking space U+00A0 = C2 A0), which orphaned the C2 lead
    -- byte and rendered it as U+FFFD "" (e.g. the smart-typography nbsp
    -- Pandoc inserts after abbreviations like "Ph.D."). Re-apply if the
    -- quarto-graph extension is updated. Upstream report recommended.
    local s, e = text:find("[ \t\r\n\f\v]+", i)
    if s == i then
