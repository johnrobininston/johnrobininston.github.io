...
In probability theory a stochastic process is said to satisfy the _Markov property_ if its future evolution is _independent of its history_, that is, the process is _memoryless_.  Consider a discrete time stochastic process $\{X_t\}_{t\in\mathbb{N}}$ defined in the probability space $(\Omega,\mathcal{F}, \mathbb{P})$.

::: {.definition data-title="Markov Property"}
...
The transition probability matrix is _stochastic_ which means that for all $n,t\in\mathbb{N}$ we have:

1. $0\leq p_{i,j}^{(n)}(t)<1$ for all $i,j\in \mathcal{X}$,
2. $\sum_{{j\in S}}p_{{i,j}}^{(n)}(t)=1$ for all $i\in \mathcal{X}$ (the rows sum to 1).

To simplify notation we denote the _one-step transition probablity matrix_
$$
P(t)=(p_{i,j}(t))_{i,j\in\mathcal{X}}.
$$

The transition probabilities are _stationary_ when they are invariant in time, that is
$$
...
::: {.example data-title="Transition Matrix"}
Consider a homogenous MC $\{X_t\}$ with finite state space $\mathcal{S}:=\{1,2,3\}$ with transition probability matrix
$$
P = \begin{bmatrix}
0 & 0.6 & 0.2 & 0.2 \\
0.75 & 0 & 0.25 & 0 \\
...
To gain deeper insight into the behavior of the chain we typically will produce _transition probability diagram_ which takes the form of a directed graph with nodes denoting states and weighted directed arrows the transition probabilities.  The transition probability diagram for our example is

![Transition probability diagram.](../assets/images/transition-probability-diagram.png){width=50%}

There are several alternate versions of this diagram including the _integer weighted diagram_. To construct the integer-weighted diagram we compute the inter-weighted matrix by multiplying elements of each row by the lowest common multiple for that row.  The integer weighted matrix for our example is
$$
P = \begin{bmatrix}
0 & 3 & 1 & 1 \\
3 & 0 & 1 & 0 \\
...
![Integer-weighted transition probability diagram.](../assets/images/iw-transition-probability-diagram.png){width=50%}

Since our diagram has a symmetric integer-weighted transition matrix we can also write the _non-directed version_ by combining the equally weighted directed arrows into a single non-directed line

![Integer-weighted non-directed ransition probability diagram.](../assets/images/iwnd-transition-probability-diagram.png){width=50%}
...
_Finite-dimensional distributions_ allow us to evaluate the behavior of the distribution of infinite length processes by selecting some sub-sequence of index values to evaluate their joint distribution.

::: {.theorem data-title="Finite-Dimensional Distributions"}
...
::: {.theorem data-title="Multi-Step Transition Probabilities for Homogenous MCs"}
...
::: {.example data-title="Multi-Step Probabilities"}
Consider a homogenous MC with state space $\mathcal{S}:=\{ 1,2,3 \}$ with transition probability matrix $P$ and initial state distribution $\mu$ given respectively by
$$
P:=\begin{bmatrix}
0.4 & 0.6 & 0 \\
0.7 & 0.3 & 0  \\
...
![Transition Probability Diagram.](../assets/images/state-diagram.jpeg){width=50%}

Suppose we wish to determine the $3$-step transition probabilities.  Applying the multi-step transition result we can compute $\mathbb{P}(X_3|X_0)$ as $P^3$:
```{python}
P = np.array([
    [.4, .6, 0],
    [.7, .3, 0],
...
::: {.definition data-title="Persistent States"}
...
A sub-class of persistent states are _null-persistent states_ which are persistent thats that have an infinite mean recurrence time.  States that are not null-persistent are said to be _positive_persistent_.
...
::: {.definition data-title="Transient States"}
...
::: {.definition data-title="Periodic States"}
...
::: {.definition data-title="Ergodic States"}
A state $j$ is _ergodic_ if it is positive persistent and aperiodic i.e. $\mu_{i,i}<\infty$ and $d(i)=1$.
:::

::: {.definition data-title="Absorbing States"}
A state $j$ is _absorbing_ if $p_{j,j}=1$, that is the probability of leaving state $j$ once the process has entered is 0.
:::

### Chain Classification

To classify chains we first define state _communication_, that is when paths between states have non-zero probability.

::: {.definition data-title="Communicating States"}
...
We can show that intercommunication is an _equivalence class_ and thus can be used to split MC state spaces into communication classes.  For $i,j$ in the same communication class we have:
...
::: {.definition data-title="Irreducibility"}
A set of states $C$ is _irreducible_ if for all $i,j\in C$, $i\leftrightarrow j$, that is all states within $C$ inter-communicate.
:::

::: {.definition data-title="Closed"}
A set of states $C$ is _closed_ if for all $i \in C$, $p_{i,j}=0$ for all $j\not\in C$, that is, the chain never leaves $C$ once it has entered.  Not that clearly the set consisting of one absorbing state is closed.
:::

::: {.example data-title="Communication Classes"}
Consider the MC transition diagram shown in the figure below.

![Example transition probability diagram.](../assets/images/complicated-transition-graph.png){width=50%}
...
The _Chapman-Kolmogorov Equations (CKE)_ relate the joint probability distributions of different sets of coordinates on a stochastic process.

::: {.theorem data-title="Chapman-Kolmogorov Equations"}
...
::: {.definition data-title="Limiting Distribution"}
...
::: {.example data-title="Limiting Behaviour"}
Reconsider our last MC example with transition diagram given in the figure below.

![Complicated transition diagram.](../assets/images/complicated-transition-graph.png){width=50%}
...
2. If the chain ends up in closed irreducible $S_3$, what is the behavior of the state distribution in the limit? $\rightarrow$ _Stationary Distributions_
:::
...
::: {.definition data-title="Strictly Stationary Process"}
...
::: {.definition data-title="Stationary Distribution"}
...
::: {.theorem data-title="Existence of a Stationary Distribution"}
...
::: {.example data-title="Restaurant Meals"}
Consider a Markov chain describing the meals served by a restaurant with transition graph shown below in Figure 1 provided by helpful video on Markov chains by [Normalized Nerd](https://www.youtube.com/watch?v=i3AkTO9HLXo&t=110s).

![Example transition diagram for a MC describing meals served at a restaurant.](../assets/images/food_example.png){width=50%}
...
see the stationary distribution definition above.  Additionally, since $\pi$ is a vector of probabilities we have that $\pi(1)+\pi(2)+\pi(3)=1$ and solving this system gives the stationary distribution
$$
\pi=\begin{bmatrix}
...
::: {.definition data-title="Doubly Stochastic Matrices"}
...
::: {.theorem data-title="Uniform Stationary Distributions for Doubly Stochastic MCs"}
