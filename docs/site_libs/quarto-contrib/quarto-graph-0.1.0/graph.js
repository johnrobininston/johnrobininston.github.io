...
(function () {
  ...
  // mkdocs-material uses .md-sidebar--secondary. For Quarto sites, anchor
  // next to the real #TOC element wherever it actually rendered (right
  // margin, left-docked sidebar, or the left-body/right-body variants),
  // rather than assuming a single container id. #quarto-margin-sidebar is
  // only a last-resort fallback: it's Quarto's general-purpose margin
  // column (asides, footnotes) and exists on the page even when
  // toc-location moves the TOC elsewhere, so matching it unconditionally
  // strands the panel on the right when the TOC itself is on the left.
  function findPanelMount() {
    var mkdocs = document.querySelector(".md-sidebar--secondary .md-sidebar__scrollwrap");
    if (mkdocs) return { container: mkdocs, before: mkdocs.firstChild };
    // Anchor directly before the TOC nav itself, not just "somewhere in
    // its parent" — on a docked left sidebar the TOC shares its parent
    // with the whole site nav menu, and inserting at the parent's
    // firstChild would strand the panel above the site title/search box
    // instead of next to the TOC.
    var toc = document.getElementById("TOC");
    if (toc && toc.parentElement) return { container: toc.parentElement, before: toc };
    var margin = document.querySelector("#quarto-margin-sidebar");
    if (margin) return { container: margin, before: margin.firstChild };
    return null;
  }
  ...
  function mountLocalPanel(data, cur, depth) {
    ...
    mount.container.insertBefore(panel, mount.before);
